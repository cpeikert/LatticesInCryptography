...
\[ f(x) = \int_{w\in \R} \hat{f}(w) \exp(2 \pi ixw) \odif{w}.
...
\begin{definition}
  For a lattice $\lat \subset \R^{n}$ and a function $g \colon \R^n/\lat \to \C$, its \emph{Fourier series} $\hat{g} \colon \lat^{*} \to \C$ is defined as
...
\begin{lemma}%
  \label{lem:g-close}
  For all $\vecx \in \R^{n}$, we have that $g(\vecx + \lat) \geq \exp(-\pi \dist(\vecx,\lat)^2)$.
\end{lemma}
