...
\begin{definition}%
  ...
  Equivalently, find an element of the lattice coset $\vect - \lat = \vect + \lat$ having norm at most $\gamma(n) \cdot \lambda(\vect+\lat)$, where $\lambda(\vect+\lat) := \min_{\vecx \in \vect+\lat} \length{\vecx} = \dist(\vect,\lat)$.
\end{definition}

In the final equality, we have used the fact that $\dist(\vect, \lat) = \min_{\vecv \in \lat} \length{\vect - \vecv} = \min_{\vecx \in \vect+\lat} \length{\vecx}$, because $\lat = -\lat$.
...
\begin{theorem}
  For any $\gamma \geq 1$, we have that $\svp_{\gamma} \leq \cvp_{\gamma}$ via a Cook reduction.
\end{theorem}
...
\begin{proof}
  ...
  Now by hypothesis on~$\Ora$, for every~$i$ we have that $\vecv_{i} \in \vecb_{i} + \lat_{i} \subset \lat \setminus \set{\veczero}$ (so the reduction outputs a nonzero lattice vector) and $\length{\vecv_{i}} \leq \gamma \cdot \lambda(\vecb_{i} + \lat_{i})$.
...
\begin{proof}
  ...
  Because $\alpha = 2 \inner{\vect, \vecv}/\inner{\vecv, \vecv}$,
  \begin{equation}
...
Here we use \cref{fact:voronoi}, which says that every relevant vector $\vecv \in \lat_{i}$ is the (unique, up to sign) shortest vector of some coset $\vect + 2\lat_{i}$ for nonzero $\vect \in \lat_{i}$.
So if we find a shortest vector of each of the~$2^{i}-1$ such cosets $\vect + 2\lat_{i}$, we will find every relevant vector.
...
Overall, to find the relevant vectors of~$\lat_{i}$ given those of~$\lat_{i-1}$, we need to solve $\cvp$ on $2^{i} - 1 = 2^{O(n)}$ cosets of $2\lat_{i}$, each of which reduces to solving $\cvp$ on $2^{O(n)}$ cosets of $2\lat_{i-1}$, each of which takes $2^{O(n)}$ time using the ``walk'' step.
